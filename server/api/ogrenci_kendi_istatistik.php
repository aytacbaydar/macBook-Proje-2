...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    try {
        ...
        $conn = getConnection();
        $ogrenci_id = $user['id']; // Kendi ID'sini kullan
        ...
        // Toplam ders sayısını hesapla (sadece normal ders ve ek ders)
        $totalLessonsCount = 0;
        foreach ($attendanceRecords as $record) {
            // Sadece normal ders ve ek ders kayıtlarını say
        ...
        $statistics = [
            ...
            'attendance_stats' => [
                'total_lessons' => $totalLessonsCount, // Sadece normal ders ve ek ders
