...
@Component({
  selector: 'app-grup-detay-sayfasi',
  templateUrl: './grup-detay-sayfasi.component.html',
  styleUrls: ['./grup-detay-sayfasi.component.scss'],
  standalone: false
})
export class GrupDetaySayfasiComponent implements OnInit {
  grupAdi: string = '';
      ...
      .subscribe({
        next: (response) => {
          if (response.success) {
            ...
            this.grupDetay.students = response.data.filter((student: any) => 
              student.grubu === this.grupAdi
            );
  ...
  loadDersKayitlari(): void {
    ...
    this.http.get<any>(`./server/api/grup_ders_kayitlari.php?grup=${encodeURIComponent(this.grupAdi)}`, { headers })
      .subscribe({
        next: (response) => {
          if (response.success) {
  ...
  viewDrawingPng(fileName: string): void {
    if (fileName) {
      ...
      const pngUrl = `${this.apiBaseUrl}/png_viewer.php?file=${encodeURIComponent(pngFileName)}`;
