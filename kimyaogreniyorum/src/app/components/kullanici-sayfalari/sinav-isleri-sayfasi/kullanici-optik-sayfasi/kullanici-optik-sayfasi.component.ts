...
@Component({
  selector: 'app-kullanici-optik-sayfasi',
  standalone: false,
  templateUrl: './kullanici-optik-sayfasi.component.html',
  styleUrl: './kullanici-optik-sayfasi.component.scss',
})
export class KullaniciOptikSayfasiComponent implements OnInit {
  sinavId: number = 0;
  sinavAdi: string = '';
  sinavTuru: string = '';
  soruSayisi: number = 0;
      ...
      next: (response: any) => {
        if (response.success) {
            ...
            setTimeout(() => {
              ...
              this.router.navigate(['/kullanici-sayfasi/sinav-sonuclari-sayfasi'], {
                queryParams: {
  ...
  goBack() {
    ...
    if (result) {
      this.router.navigate([
        '/kullanici-sayfasi/sinav-isleri-sayfasi',
      ]);
    }
  }
