...
export class OgretmenOgrenciBilgiSayfasiComponent
  implements OnInit, AfterViewInit, OnDestroy
{
  @ViewChild('sinavChart', { static: false })
  sinavChartRef?: ElementRef<HTMLCanvasElement>;
  ...
  async loadAllData(): Promise<void> {
    ...
    try {
      ...
      if (this.ogrenciBilgileri) {
        ...
        console.log(
          '✓ Katılım verileri yüklendi:',
          this.historicalAttendance.length,
          'kayıt'
        );
      }
  ...
  loadSinavSonuclari(): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      this.http
        .get<any>(
          `server/api/ogrenci_tum_sinav_sonuclari.php?ogrenci_id=${this.ogrenciId}`,
          { headers }
        )
        .subscribe({
          next: (response) => {
            if (response && response.success && response.data) {
              ...
              if (
                response.data.sinav_sonuclari &&
                Array.isArray(response.data.sinav_sonuclari)
              ) {
                // Öğrenci ID'ye göre filtrele
                const filteredResults = response.data.sinav_sonuclari.filter(
                  (sinav: any) =>
                    sinav &&
                    sinav.ogrenci_id &&
                    sinav.ogrenci_id == this.ogrenciId
                );
                ...
                this.sinavSonuclari = filteredResults.map((sinav: any) => ({
                  ...
                  toplam_soru:
                    (sinav.dogru_sayisi || 0) +
                    (sinav.yanlis_sayisi || 0) +
                    (sinav.bos_sayisi || 0),
                }));
              } else {
                this.sinavSonuclari = [];
  ...
  loadKonuAnalizleri(): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      this.http
        .get<any>(
          `server/api/ogrenci_konu_analizi.php?ogrenci_id=${this.ogrenciId}`,
          { headers }
        )
        .subscribe({
          next: (response) => {
            if (response && response.success && response.data) {
              ...
              if (Array.isArray(response.data.konu_istatistikleri)) {
                ...
                response.data.konu_istatistikleri &&
                typeof response.data.konu_istatistikleri === 'object'
              ) {
                // Object ise array'e çevir
                this.konuAnalizleri = Object.values(
                  response.data.konu_istatistikleri
                ).filter(
                  (item: any) =>
  ...
  loadOdemeBilgileri(): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      this.http
        .get<any>(
          `server/api/ogretmen_ucret_yonetimi.php?ogrenci_id=${this.ogrenciId}`,
          { headers }
        )
        .subscribe({
          next: (response) => {
            if (response && response.success) {
              ...
              if (
                ...
                this.odemeBilgileri = response.data.payments.filter(
                  (payment: any) => payment.ogrenci_id == this.ogrenciId
                );
              } else if (Array.isArray(response.data)) {
                this.odemeBilgileri = response.data.filter(
                  (payment: any) => payment.ogrenci_id == this.ogrenciId
                );
              } else {
                this.odemeBilgileri = [];
          ...
          error: (error) => {
            ...
            console.warn(
              'Ödeme bilgileri yüklenemedi, boş array ile devam ediliyor'
            );
            this.odemeBilgileri = [];
            resolve(); // Hata olsa da devam et
          },
  ...
  calculateStatistics(): void {
    ...
    this.devamsizlikSayisi = Array.isArray(this.historicalAttendance)
      ? this.historicalAttendance.filter(
          (kayit: any) =>
            kayit &&
            kayit.durum === 'absent' &&
            kayit.ogrenci_id === this.ogrenciBilgileri?.id
        ).length
      : 0;
    ...
    if (Array.isArray(this.sinavSonuclari) && this.sinavSonuclari.length > 0) {
      this.ortalamaPuan =
        this.sinavSonuclari.reduce(
          (total, sinav) => total + (sinav.puan || 0),
          0
        ) / this.sinavSonuclari.length;
    } else {
      this.ortalamaPuan = 0;
    }
  ...
  prepareChartData(): void {
    if (Array.isArray(this.sinavSonuclari) && this.sinavSonuclari.length > 0) {
      ...
      const sortedResults = [...this.sinavSonuclari].sort(
  ...
  renderChart(): void {
    ...
    try {
      ...
      const basariOranlari = this.sinavSonuclari.map((sinav) =>
        this.getSuccessPercentage(sinav)
      );
  ...
  loadStudentAttendanceData(): Promise<void> {
    return new Promise((resolve, reject) => {
      ...
      console.log(
        `Katılım verileri yükleniyor - Öğrenci ID: ${this.ogrenciId}`
      );
      ...
      this.http.get<any>(apiUrl, { headers, params }).subscribe({
        next: (response) => {
          ...
          if (response && response.success && response.data) {
            ...
            this.devamsizlikKayitlari = this.historicalAttendance.map(
              (record: any) => ({
                id: record.id || 0,
                tarih: record.tarih || '',
                durum: record.durum || '',
            ...
            console.log('Başarıyla yüklenen katılım verileri:', {
              ...
              presentCount: this.historicalAttendance.filter(
                (r) => r.durum === 'present'
              ).length,
              absentCount: this.historicalAttendance.filter(
                (r) => r.durum === 'absent'
              ).length,
              student42Records: this.historicalAttendance.filter(
                (r) => r.ogrenci_id == 42
              ).length,
            });
  ...
  getPaymentAnalysisData(): any[] {
    ...
    const ekDersLessons = studentRecords.filter(
      (record: any) =>
        record.durum === 'present' && record.ders_tipi === 'ek_ders'
    ).length;
    ...
    const paidAmount = Array.isArray(this.odemeBilgileri)
      ? this.odemeBilgileri.reduce(
          (total, payment) => total + (payment.tutar || 0),
          0
        )
      : 0;
    ...
    const paymentPercentage =
      expectedAmount > 0 ? Math.round((paidAmount / expectedAmount) * 100) : 0;
  ...
  getStudentAttendanceAnalysis(): any[] {
    if (
      !this.ogrenciBilgileri ||
      !this.historicalAttendance ||
      this.historicalAttendance.length === 0
    ) {
      return [];
    }
  ...
  getTotalRecordsCount(): number {
    return Array.isArray(this.historicalAttendance)
      ? this.historicalAttendance.length
      : 0;
  }
