...
export class OgrenciAnaSayfasiComponent implements OnInit {
  ...
  private loadLessonStats(): void {
    ...
    // Önce sınıf seviyesine uygun toplam konuları yükle
    this.loadFilteredTopicsForClass().then((classTopics) => {
      ...
      this.http.get<any>(apiUrl).subscribe({
        next: (response) => {
          if (response.success && response.islenen_konular) {
            ...
            this.totalLessons = classTopics.length; // Sınıfa özel toplam konu sayısı
            
            console.log('Ders istatistikleri yüklendi:', {
              ogrenciSinifi: this.studentClass,
              sinifaOzelToplamKonu: this.totalLessons,
      ...
      console.error('Sınıf bazlı konu listesi yüklenirken hata:', error);
    });
  }

  // Sınıf seviyesine göre filtrelenmiş konu sayısını al (işlenen konular sayfasıyla aynı mantık)
  private loadFilteredTopicsForClass(): Promise<any[]> {
      ...
      this.http.get<any>(`${this.apiBaseUrl}/konu_listesi.php`, { headers }).subscribe({
        next: (response) => {
          if (response.success && response.konular) {
            ...
            // Sınıf seviyesine göre filtrele (işlenen konular sayfasıyla aynı mantık)
            if (studentClass) {
